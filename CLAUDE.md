...
**Financeiro do PSH (Polpa Santa Helena) é separado do financeiro pessoal**: a tabela `gastos` é pessoal e **nunca** recebe lançamento do negócio — misturar compras de polpa nas categorias pessoais destruiria os relatórios de `/financeiro`. O negócio vive em `psh_lancamentos` (`tipo`: `venda` | `compra` | `despesa`, `migrations/010_psh_financeiro.sql`), e `estoque_produtos.preco_compra` é o custo (o `preco` que já existia é o preço de venda).

**Venda/compra do PSH não são digitadas duas vezes — um trigger no banco as cria**: `trg_psh_lancamento_de_movimentacao` (AFTER INSERT em `estoque_movimentacoes`) gera o lançamento financeiro de toda movimentação `venda` (receita = qtd × `preco`) e `entrada` (compra = qtd × `preco_compra`). `transferencia`, `contagem` e `consumo` não movimentam dinheiro e não geram lançamento. É trigger e não código porque hoje há 3 caminhos que gravam movimentação (frontend individual, frontend em lote, chat) e cada caminho novo esqueceria de lançar o financeiro. **Consequência prática**: nunca inserir receita/custo à mão depois de gravar uma movimentação — recarregue `psh_lancamentos` do banco (é o que `carregarLancamentos()` faz em `Estoque/index.jsx`). Corrigir o valor de um lançamento automático marca `editado: true`, que protege a linha de recálculo futuro.

**Estoque do freezer pode ficar negativo — de propósito**: vender antes de registrar a transferência da câmara é rotina, e o saldo negativo é justamente o que sinaliza que a transferência ainda falta. Truncar em zero (o antigo `Math.max(0, ...)` em `registrarMovimentacao`/`registrarGerenciar` e em `confirmarVendaPSH`) apagava essa informação e fazia a transferência seguinte somar em cima de um número errado, então o estoque nunca fechava. **A câmara fria continua travada em zero**: não faz sentido tirar de onde não entrou — se o pedido chegou, registre a entrada primeiro. Essa assimetria é intencional; as guardas `excedeCamara` (`ModalMovimentacao`, `ModalGerenciar`) e `semSaldo` (`index.jsx`, `handlers/psh.js`) existem só para o lado da câmara.

**Lucro ≠ caixa na apuração do PSH**: o lucro usa o **CMV** (custo só do que foi *vendido* = quantidade vendida × `preco_compra`), não o total comprado no período — senão um mês de pedido grande apareceria como prejuízo. O caixa (entrou − saiu) é mostrado à parte. Essa mesma apuração está duplicada de propósito em dois lugares que não compartilham runtime: `apurar()` em `Estoque/AbaFinanceiro.jsx` (frontend) e `responderConsulta()` em `handlers/psh.js` (chat) — ao mudar a regra, mudar nos dois.
